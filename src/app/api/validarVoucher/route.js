...
export async function POST(req) {
  try {
    ...
    console.log(`📡 Validando voucher ${couponCode} para cliente ${clientId}...`);
    ...
    // 2️⃣ Verificar se o voucher existe e pertence ao parceiro logado
    const voucherQuery = `
    ...
    // 3️⃣ Verificar se o voucher pertence ao parceiro logado
    if (voucher.parceiro_email !== session.user.email) {
      return new Response(JSON.stringify({ error: "Você não tem permissão para validar este voucher." }), { status: 403 });
    }
    ...
    // 5️⃣ Registrar o uso do voucher (SEM parceiro_id)
    ...
    console.log(`✅ Voucher ${couponCode} validado para cliente ${clientId}`);
