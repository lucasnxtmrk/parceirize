-- Migração segura para subdomínios - adiciona apenas o que não existe

-- 1. Adicionar coluna subdominio apenas se não existir
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE provedores ADD COLUMN subdominio VARCHAR(50) UNIQUE;
    RAISE NOTICE 'Coluna subdominio adicionada com sucesso';
  ELSE
    RAISE NOTICE 'Coluna subdominio já existe';
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_provedores_subdominio'
  ) THEN
    CREATE INDEX idx_provedores_subdominio ON provedores(subdominio);
    RAISE NOTICE 'Índice idx_provedores_subdominio criado';
  ELSE
    RAISE NOTICE 'Índice idx_provedores_subdominio já existe';
  END IF;
END $$;

-- 3. Função para validar subdomínio (sempre recriar para garantir versão atualizada)
...
-- 4. Função para criar domínio automaticamente
...
-- 5. Função para atualizar domínio quando subdomínio muda
...
-- 7. Função helper para buscar provedor por subdomínio
...
-- 8. Atualizar provedores de teste existentes (cuidado para não duplicar)
DO $$
BEGIN
  -- Atualizar apenas se não tiver subdomínio ainda
  UPDATE provedores
  SET subdominio = 'teste'
  WHERE id = 1 AND (subdominio IS NULL OR subdominio = '');

  UPDATE provedores
  SET subdominio = 'empresa1'
  WHERE id = 2 AND (subdominio IS NULL OR subdominio = '');

  RAISE NOTICE 'Provedores de teste atualizados';
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Erro ao atualizar provedores de teste: %', SQLERRM;
END $$;

RAISE NOTICE 'Migração segura de subdomínios concluída!';