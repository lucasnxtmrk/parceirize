-- Corrigir função que causa erro de duplicação de domínio
-- Problema: função atualizar_dominio_automatico() tenta inserir domínio que já existe
...
BEGIN
  ...
  IF OLD.subdominio IS DISTINCT FROM NEW.subdominio THEN
    ...
    IF NEW.subdominio IS NOT NULL AND LENGTH(TRIM(NEW.subdominio)) > 0 THEN
      ...
      ELSE
        -- Domínio já existe, apenas reativar se necessário
        UPDATE dominios_personalizados
        SET ativo = true,
            provedor_id = NEW.id,
            atualizado_em = NOW()
...
-- Comentário das correções
COMMENT ON FUNCTION atualizar_dominio_automatico() IS 'Trigger function que atualiza domínio quando subdomínio do provedor muda - CORRIGIDO para evitar duplicatas';
COMMENT ON FUNCTION criar_dominio_automatico() IS 'Trigger function que cria domínio automaticamente quando provedor é inserido - CORRIGIDO para evitar duplicatas';